...
# The GTK+4 theme no longer works, so if gtkrc uses that, back up the old
# file and install the new one:
if grep -q GTK etc/gtk-4.0/gtkrc 2> /dev/null ; then
  mv etc/gtk-4.0/gtkrc etc/gtk-4.0/gtkrc.bak
fi

config usr/share/gtk-4.0/settings.ini.new
...
if [ -x /usr/bin/update-gtk-immodules-4.0 ]; then
  /usr/bin/update-gtk-immodules-4.0
fi
